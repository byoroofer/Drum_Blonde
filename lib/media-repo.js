...
function getRowDurationMs(row) {
  if (row?.duration_ms != null) {
    return Number(row.duration_ms);
  }

  if (row?.duration_seconds != null) {
    return Math.round(Number(row.duration_seconds) * 1000);
  }
...
function normalizeHomeSlot(value, fallback = null) {
  const numeric = Math.round(Number(value));
...
function getPlaybackSettingsFromLog(row) {
  ...
  const durationSeconds = durationMs == null ? null : Math.max(0, Math.round(Number(durationMs) / 1000));
...
function getLatestLoggedFields(row) {
  const logs = toJsonArray(row?.processing_log);
...
function getDerivedAssetState(row) {
  const logs = [...toJsonArray(row?.processing_log), ...toJsonArray(row?.ingestion_log)];
  ...
  for (let index = logs.length - 1; index >= 0; index -= 1) {
    ...
    if (entry.step === "derived_asset" && entry.source_asset_id) {
      return {
        derivedFromAssetId: String(entry.source_asset_id),
        derivedFromTitle: entry.source_asset_title ? String(entry.source_asset_title) : "",
        editType: entry.edit_type ? String(entry.edit_type) : "",
        coverThumbnailUrl: entry.cover_thumbnail_url ? String(entry.cover_thumbnail_url) : null,
        coverThumbnailStoragePath: entry.cover_thumbnail_storage_path ? String(entry.cover_thumbnail_storage_path) : null,
        editCreatedAt: entry.at || null
      };
    }
  }
  ...
  return {
    ...
    coverThumbnailStoragePath: null,
    editCreatedAt: null
  };
}

function getLatestDerivedSummary(row) {
  const logs = toJsonArray(row?.processing_log);
  ...
  for (let index = logs.length - 1; index >= 0; index -= 1) {
    ...
    if (entry.step === "derived_asset_created" && entry.derived_asset_id) {
      return {
        latestDerivedAssetId: String(entry.derived_asset_id),
        latestDerivedEditType: entry.edit_type ? String(entry.edit_type) : "",
        latestDerivedAt: entry.at || null,
        latestDerivedUrl: entry.public_url ? String(entry.public_url) : null
      };
    }
  }

  return {
    latestDerivedAssetId: null,
    latestDerivedEditType: "",
    latestDerivedAt: null,
    latestDerivedUrl: null
  };
}

function getSpotlightSelectionState(row) {
  const fields = getLatestLoggedFields(row);

  if (fields && Object.prototype.hasOwnProperty.call(fields, "spotlight_home")) {
    return fields.spotlight_home === true;
  }

  return isExplicitSpotlightSlot(row?.home_slot);
}
...
async function normalizeRow(row, albumLookup = new Map()) {
  ...
  const derivedState = getDerivedAssetState(row);
  const latestDerivedSummary = getLatestDerivedSummary(row);
  ...
  const item = {
    ...
    durationMs: getRowDurationMs(row),
    durationSeconds: getRowDurationMs(row) == null ? null : Math.round(Number(getRowDurationMs(row)) / 1000),
    ...
    spotlightHome: getSpotlightSelectionState(row),
    ...
    derivedFromAssetId: derivedState.derivedFromAssetId,
    derivedFromTitle: derivedState.derivedFromTitle,
    editType: derivedState.editType,
    coverThumbnailUrl: derivedState.coverThumbnailUrl,
    coverThumbnailStoragePath: derivedState.coverThumbnailStoragePath,
    editCreatedAt: derivedState.editCreatedAt,
    latestDerivedAssetId: latestDerivedSummary.latestDerivedAssetId,
    latestDerivedEditType: latestDerivedSummary.latestDerivedEditType,
    latestDerivedAt: latestDerivedSummary.latestDerivedAt,
    latestDerivedUrl: latestDerivedSummary.latestDerivedUrl,
    isDerived: Boolean(derivedState.derivedFromAssetId),
...
function buildDerivedFilename(originalFilename, preferredExtension = "") {
  const base = stripExtension(originalFilename || "asset");
  const extension = preferredExtension || path.extname(originalFilename || "") || ".bin";
  return `${base}-edited${extension}`;
}

function buildDerivedTitle(title) {
  const baseTitle = String(title || "Untitled asset").trim() || "Untitled asset";
  if (/\(edited/i.test(baseTitle)) {
    return baseTitle;
  }
...
async function insertMediaVersionRecord(values) {
  const supabase = createSupabaseAdminClient();

  try {
    const result = await supabase.from("media_versions").insert(values);
    assertSupabaseResult("insert_media_version", result);
    return true;
  } catch (error) {
    console.error("[media-engine] media_versions insert skipped", error);
    return false;
  }
}

async function persistDerivedMediaAsset({
  sourceAssetRow,
  outputBuffer,
  outputMimeType,
  outputFilename,
  ...
  const createdAsset = await ingestUploadedMedia({
    buffer: outputBuffer,
    mimeType: outputMimeType,
    originalFilename: outputFilename,
    source: "editor",
    title: buildDerivedTitle(sourceAssetRow.title || stripExtension(sourceAssetRow.original_filename)),
    description: sourceAssetRow.description || "",
    tags: sourceAssetRow.tags || "",
    workflowStatus: sourceAssetRow.workflow_status || "approved",
    active: sourceAssetRow.is_active !== false
  });

  const derivedRow = await getMediaRowById(createdAsset.id);
  const uploadedPaths = [];
  let nextThumbnailStoragePath = derivedRow.thumbnail_storage_path || null;
  let nextThumbnailUrl = derivedRow.thumbnail_url || null;

  try {
    if (coverBuffer?.length) {
      const coverFilename = `${stripExtension(outputFilename)}-cover.jpg`;
      const coverStoragePath = buildStoragePath({
        source: "editor",
        filename: coverFilename
      });

      await uploadToStorage(coverStoragePath, coverBuffer, coverMimeType || "image/jpeg");
      uploadedPaths.push(coverStoragePath);

      nextThumbnailStoragePath = coverStoragePath;
      nextThumbnailUrl = usePublicMediaUrls()
        ? createSupabaseAdminClient().storage.from(getMediaBucketName()).getPublicUrl(coverStoragePath).data.publicUrl
        : null;
    }

    const derivedLogEntry = createLogEntry("derived_asset", "completed", {
      source_asset_id: sourceAssetRow.id,
      source_asset_title: sourceAssetRow.title || stripExtension(sourceAssetRow.original_filename) || "Untitled asset",
      edit_type: editType,
      cover_time_seconds: coverTimeSeconds,
      cover_thumbnail_storage_path: nextThumbnailStoragePath,
      cover_thumbnail_url: nextThumbnailUrl,
      output_filename: outputFilename,
      output_mime_type: outputMimeType,
      output_file_size_bytes: outputBuffer.length,
      output_width: outputMetadata?.width ?? null,
      output_height: outputMetadata?.height ?? null,
      output_duration_ms: outputMetadata?.durationMs ?? null,
      edit_payload: editPayload
    });

    const updatedDerivedRow = await writeMediaUpdate(createdAsset.id, {
      album_slugs: toStringArray(sourceAssetRow.album_slugs).map((value) => sanitizePathSegment(value)),
      featured_home: false,
      home_slot: null,
      manual_rank: 0,
      thumbnail_storage_path: nextThumbnailStoragePath,
      thumbnail_url: nextThumbnailUrl,
      ingestion_log: [...toJsonArray(derivedRow.ingestion_log), derivedLogEntry],
      processing_log: [
        ...toJsonArray(derivedRow.processing_log),
        createLogEntry("derived_asset_link", "completed", {
          source_asset_id: sourceAssetRow.id,
          edit_type: editType
        })
      ]
    ...
    const versionInserted = await insertMediaVersionRecord({
      media_asset_id: sourceAssetRow.id,
      version_kind: "derived_edit",
      variant_label: `${editType}-${createdAsset.id}`,
      storage_path: updatedDerivedRow.storage_path,
      public_url: updatedDerivedRow.public_url,
      width: outputMetadata?.width ?? null,
      height: outputMetadata?.height ?? null,
      duration_seconds: outputMetadata?.durationMs == null ? null : Math.round(Number(outputMetadata.durationMs) / 1000),
      file_size_bytes: outputBuffer.length,
      transformation_status: "ready",
      transformation_notes: JSON.stringify({
        source_asset_id: sourceAssetRow.id,
        derived_asset_id: createdAsset.id,
        edit_type: editType,
        cover_thumbnail_url: nextThumbnailUrl,
        cover_time_seconds: coverTimeSeconds,
        created_at: nowIso()
    ...
    await writeMediaUpdate(sourceAssetRow.id, {
      processing_log: [
        ...toJsonArray(sourceAssetRow.processing_log),
        createLogEntry("derived_asset_created", "completed", {
          derived_asset_id: createdAsset.id,
          edit_type: editType,
          public_url: updatedDerivedRow.public_url,
          storage_path: updatedDerivedRow.storage_path,
          media_version_recorded: versionInserted
        })
      ]
    });

    if (coverBuffer?.length && derivedRow.thumbnail_storage_path && derivedRow.thumbnail_storage_path !== nextThumbnailStoragePath) {
      await removeFromStorage(derivedRow.thumbnail_storage_path).catch(() => {});
    }

    return normalizeRow(updatedDerivedRow);
  } catch (error) {
...
export async function getAdminMediaAsset(id) {
  const mediaId = String(id || "").trim();
  if (!mediaId) {
    throw buildStructuredError("select_media_asset", new Error("Media id is required."));
  }

  const row = await getMediaRowById(mediaId);
  const allRows = await listRows();
  const albumLookup = await getAlbumLookupForRows(allRows);
  return normalizeRow(row, albumLookup);
}

export async function getMediaAssetSource(id) {
  const mediaId = String(id || "").trim();
  if (!mediaId) {
    throw buildStructuredError("download_media_source", new Error("Media id is required."));
  }

  const row = await getMediaRowById(mediaId);
  if (!row.storage_path) {
    throw buildStructuredError("download_media_source", new Error("Media source is missing a storage path."));
  }

  const file = await downloadFromStorage(row.storage_path);
  return {
    row,
    filename: row.original_filename || `${mediaId}.bin`,
    mimeType: row.mime_type || "application/octet-stream",
    buffer: Buffer.from(await file.arrayBuffer())
  };
}

export async function createDerivedMediaEdit({
  id,
  fileBuffer,
  mimeType,
  originalFilename,
  ...
  const mediaId = String(id || "").trim();
  if (!mediaId) {
    throw buildStructuredError("create_derived_media_edit", new Error("Media id is required."));
  }

  if (!fileBuffer || !Buffer.isBuffer(fileBuffer) || !fileBuffer.length) {
    throw buildStructuredError("create_derived_media_edit", new Error("Edited media file is required."));
  }

  const sourceAssetRow = await getMediaRowById(mediaId);

  return persistDerivedMediaAsset({
    sourceAssetRow,
    outputBuffer: fileBuffer,
    outputMimeType: mimeType,
    outputFilename: originalFilename || buildDerivedFilename(sourceAssetRow.original_filename, path.extname(sourceAssetRow.original_filename || "")),
    editType: String(editType || (String(mimeType || "").startsWith("video/") ? "video" : "image")),
    editPayload,
    outputMetadata: { width, height, durationMs },
    coverBuffer,
    coverMimeType,
    coverTimeSeconds
  });
...
export async function duplicateMediaAsset(id) {
  const mediaId = String(id || "").trim();
  if (!mediaId) {
    throw buildStructuredError("duplicate_media_asset", new Error("Media id is required."));
  }

  const sourceAssetRow = await getMediaRowById(mediaId);
  if (!sourceAssetRow.storage_path) {
    throw buildStructuredError("duplicate_media_asset", new Error("Media asset is missing a storage path."));
  }

  const sourceFile = await downloadFromStorage(sourceAssetRow.storage_path);
  const sourceBuffer = Buffer.from(await sourceFile.arrayBuffer());
  const duplicatedAsset = await ingestUploadedMedia({
    buffer: sourceBuffer,
    mimeType: sourceAssetRow.mime_type || "application/octet-stream",
    originalFilename: sourceAssetRow.original_filename || `${mediaId}.bin`,
    source: "duplicate",
    title: `${sourceAssetRow.title || stripExtension(sourceAssetRow.original_filename) || "Untitled asset"} Copy`,
    description: sourceAssetRow.description || "",
    tags: sourceAssetRow.tags || "",
    workflowStatus: sourceAssetRow.workflow_status || "approved",
    active: sourceAssetRow.is_active !== false
  ...
  const duplicateRow = await getMediaRowById(duplicatedAsset.id);
  const duplicateLogEntry = createLogEntry("duplicate_asset", "completed", {
    source_asset_id: sourceAssetRow.id,
    source_asset_title: sourceAssetRow.title || stripExtension(sourceAssetRow.original_filename) || "Untitled asset"
  });

  const updatedDuplicateRow = await writeMediaUpdate(duplicatedAsset.id, {
    album_slugs: toStringArray(sourceAssetRow.album_slugs).map((value) => sanitizePathSegment(value)),
    featured_home: false,
    home_slot: null,
    manual_rank: 0,
    ...
    override_by: null,
    override_notes: null,
    ingestion_log: [...toJsonArray(duplicateRow.ingestion_log), duplicateLogEntry],
    processing_log: [
      ...toJsonArray(duplicateRow.processing_log),
      createLogEntry("duplicate_asset_link", "completed", {
        source_asset_id: sourceAssetRow.id
      })
    ]
  });
  ...
  await writeMediaUpdate(sourceAssetRow.id, {
    processing_log: [
      ...toJsonArray(sourceAssetRow.processing_log),
      createLogEntry("asset_duplicated", "completed", {
        duplicated_asset_id: duplicatedAsset.id,
        duplicated_asset_title: updatedDuplicateRow.title || duplicatedAsset.title || "Untitled asset copy"
      })
    ]
  });

  return getAdminMediaAsset(duplicatedAsset.id);
}
...
function rotateOrderedItems(items, intervalMs = 5 * 60 * 1000) {
...
function compareByFallbackSpotlightPriority(left, right) {
  if ((left.views || 0) !== (right.views || 0)) {
    return (right.views || 0) - (left.views || 0);
  }
...
function buildHomepageSelection(items) {
  ...
  const explicitSpotlightPool = [...eligibleVideos]
    .filter((item) => item.spotlightHome === true)
    .sort(compareBySpotlightPriority);
  const rotatedSpotlightPool = rotateOrderedItems(explicitSpotlightPool);
  const explicitSpotlight = rotatedSpotlightPool[0] || null;
  const fallbackSpotlight = explicitSpotlight || [...eligibleVideos].sort(compareByFallbackSpotlightPriority)[0] || orderedFeaturedItems[0] || null;
  const rotationVideos = rotateOrderedItems(
    sortByHomepagePriority(eligibleVideos.filter((item) => item.id !== fallbackSpotlight?.id))
  );
...
export async function updateMediaAsset(values) {
  ...
  const nextSpotlight = hasOwn("spotlightHome")
    ...
    : hasOwn("featuredHome") && values.featuredHome !== true
      ? false
      : getSpotlightSelectionState(currentRow);
  const nextClipSettings = sanitizeClipRange(
    ...
    getRowDurationMs(currentRow) == null ? null : Math.round(Number(getRowDurationMs(currentRow)) / 1000)
  );
  ...
  const payload = {
    ...
    home_slot: hasOwn("homeSlot")
        ? String(values.homeSlot || "").trim()
          ? normalizeHomeSlot(values.homeSlot, null)
          : null
        : currentRow.home_slot == null
          ? null
          : isExplicitSpotlightSlot(currentRow.home_slot)
            ? null
            : normalizeHomeSlot(currentRow.home_slot, null),
...
export async function applyMediaEdits(values) {
  ...
  const durationMs = getRowDurationMs(currentRow);
  const durationSeconds = durationMs == null ? null : Math.max(0, Math.round(Number(durationMs) / 1000));
  ...
  try {
    ...
    try {
      thumbnailBuffer = await generateThumbnailBuffer(editedBuffer, nextMimeType, outputPath);
    ...
    return persistDerivedMediaAsset({
      ...
      outputMimeType: nextMimeType,
      outputFilename: nextFilename,
      editType: isVideo ? "video" : "image",
      editPayload: editDetail,
      outputMetadata: metadata,
      coverBuffer: thumbnailBuffer,
      coverMimeType: "image/jpeg",
      coverTimeSeconds: isVideo ? trimSettings.clipStartSeconds : null
    });
  } catch (error) {
    await Promise.all(uploadedPaths.map((entry) => removeFromStorage(entry).catch(() => {})));
