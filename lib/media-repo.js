...
function buildHomepageSelection(items) {
  ...
  const spotlightLeader = [...eligibleVideos].sort(compareBySpotlightPriority)[0] || null;
  const rotationVideos = spotlightLeader
    ? [spotlightLeader, ...sortByHomepagePriority(eligibleVideos.filter((item) => item.id !== spotlightLeader.id))]
    : [];
  ...
  return {
    heroVideo: rotationVideos[0] || null,
    secondaryVideo: rotationVideos[1] || null,
    tertiaryVideo: rotationVideos[2] || null,
    backgroundVideos: rotationVideos.slice(3, 6),
    featuredVideos: rotationVideos,
